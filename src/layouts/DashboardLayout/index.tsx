...
import { Menubar, ProgressCustom, Notification } from '../../components';
...
export const DashboardLayout = (props: DashboardLayoutProps) => {
    ...
    const serviceState = useSelector((state: RootState) => state.service.services);
    ...
    const filterDevicesState = (type: string, value: string) => {
        ...
        if (type === 'devices') {
            if (value === "Hoạt động") res = devicesState.filter((item) => { return item.activeStatus === value })
            if (value === "Ngưng hoạt động") res = devicesState.filter((item) => { return item.activeStatus === value })
        }
        if (type === 'services') {
            if (value === "Hoạt động") res = serviceState.filter((item) => { return item.activeStatus === value })
            if (value === "Ngưng hoạt động") res = serviceState.filter((item) => { return item.activeStatus === value })
        }
        if (type === 'numberLevel') {
            if (value === "Đã sử dụng") res = numberLevelsState.filter((item) => { return item.status === value })
            if (value === "Đang chờ") res = numberLevelsState.filter((item) => { return item.status === value })
            if (value === "Bỏ qua") res = numberLevelsState.filter((item) => { return item.status === value })
        }
        return res.length;
    }
    ...
    const handleCalPercent = (type: string, value: string) => {
        ...
        if (type === 'numberLevel') result = (count / numberLevelsState.length) * 100;
                        ...
                        <div className={styles.circle} >
                            ...
                            <div>
                                <div className={styles.statusInfo} >
                                    ...
                                    <p>Đang hoạt động <span className={styles.textColorOrange} >{filterDevicesState('devices', "Hoạt động")}</span></p>
                                </div>

                                <div className={styles.statusInfo} >
                                    <p className={`${styles.status} ${styles.gray}`} ></p>
                                    <p>Ngưng hoạt động <span className={styles.textColorOrange} >{filterDevicesState('devices', "Ngưng hoạt động")}</span></p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className={styles.circle} >
                            ...
                            <div className={styles.number} >
                                ...
                                <div className={styles.service} >
                                    <svg width="16" height="15" viewBox="0 0 16 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <ellipse cx="3.22143" cy="10.0817" rx="0.498165" ry="0.495793" fill="#4277FF" />
                                        <ellipse cx="4.88183" cy="10.0817" rx="0.498165" ry="0.495793" fill="#4277FF" />
                                        <ellipse cx="6.54248" cy="10.0817" rx="0.498165" ry="0.495793" fill="#4277FF" />
                                        <g clipPath="url(#clip0_201_18563)">
                                            ...
                                            <path d="M9.63287 7.36051C9.81985 7.3659 9.95733 7.50876 9.95184 7.68665C9.94634 7.86993 9.7951 8.0047 9.60538 7.99661C9.4239 7.98853 9.28642 7.84298 9.29466 7.66509C9.30016 7.48719 9.4459 7.35512 9.63287 7.36051Z" fill="#4277FF" />
                                        </g>
                                        <path d="M14.7704 5.7304C14.7704 7.04284 14.0591 8.22267 12.9266 9.04368C12.8874 9.07098 12.8658 9.11778 12.8639 9.16459L12.8149 10.4419C12.809 10.6135 12.6189 10.713 12.4739 10.6213L11.3864 9.94074C11.3864 9.94074 11.3864 9.94074 11.3845 9.94074C11.3218 9.89978 11.2453 9.88808 11.1748 9.90954C10.5282 10.1104 9.82472 10.2216 9.08797 10.2216C9.07817 10.2216 9.06837 10.2216 9.05857 10.2216C9.07817 10.0928 9.08797 9.96219 9.08797 9.82958C9.08797 7.99841 7.2108 6.51436 4.89472 6.51436C4.41857 6.51436 3.96201 6.57676 3.53485 6.69182C3.44863 6.38175 3.40356 6.05802 3.40356 5.7265C3.40356 3.24398 5.94695 1.2334 9.08601 1.2334C12.227 1.2373 14.7704 3.24983 14.7704 5.7304Z" stroke="#4277FF" strokeWidth="1.10526" strokeMiterlimit="10" />
                                        <path d="M3.53675 6.69531C1.88884 7.14189 0.703369 8.37828 0.703369 9.83308C0.703369 10.8003 1.22851 11.6721 2.06324 12.2785C2.09263 12.3 2.1083 12.3331 2.11026 12.3682L2.14553 13.3102C2.14945 13.4369 2.29053 13.5091 2.3983 13.4428L3.20168 12.9396C3.20756 12.9357 3.2154 12.9299 3.22128 12.926C3.25067 12.9026 3.28986 12.8948 3.32513 12.9065C3.81108 13.0625 4.34013 13.1483 4.89662 13.1483C7.04419 13.1483 8.81555 11.871 9.06048 10.2251" stroke="#4277FF" strokeWidth="1.10526" strokeMiterlimit="10" />
                                        <defs>
                            ...
                            <div>
                                <div className={styles.statusInfo} >
                                    ...
                                    <p>Đang hoạt động <span className={styles.textColorblue} >{filterDevicesState('services', "Hoạt động")}</span></p>
                                </div>

                                <div className={styles.statusInfo} >
                                    <p className={`${styles.status} ${styles.gray}`} ></p>
                                    <p>Ngưng hoạt động <span className={styles.textColorblue} >{filterDevicesState('services', "Ngưng hoạt động")}</span></p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className={styles.circle} >
                            ...
                            <div className={styles.listStatus} >
                                <div className={styles.statusInfo} >
                                    ...
                                    <p>Đã sử dụng <span className={styles.textColorGreen} >{filterDevicesState('numberLevel', "Đã sử dụng")}</span></p>
                                </div>

                                <div className={styles.statusInfo} >
                                    <p className={`${styles.status} ${styles.gray}`} />
                                    <p>Đang chờ <span className={styles.textColorGreen} >{filterDevicesState('numberLevel', "Đang chờ")}</span></p>
                                </div>

                                <div className={styles.statusInfo} >
                                    <p className={`${styles.status} ${styles.pink}`} />
                                    <p>Bỏ qua <span className={styles.textColorGreen} >{filterDevicesState('numberLevel', "Bỏ qua")}</span></p>
                                </div>
                            </div>
                        </div>
                    </div>
