...
export const StreamChart = () => {
  ...
  const dataDashboardState = useSelector((state: RootState) => state.dashboard.data);
  const typeFilterDashboardState = useSelector((state: RootState) => state.dashboard.typeFilter);
  ...
  useEffect(() => {
    ...
    if(typeFilterDashboardState !== "Tuần"){
      dataDashboardState.forEach((item) => {
      ...
      dataDashboardState.forEach((item: {month: number, week: number, value: number}) => {
        convert.push({
          x: item.week,
          y: item.value
        })
