...
import { getAllReport, updateReportsFilter } from '../../store/reducers/reportSlice';
import { getOnlyDate } from '../../utils';

const headers = [
  {label: 'Số thứ tự', key: "number"},
  {label: 'Tên dịch vụ', key: "name"},
  {label: 'Thời gian cấp', key: "date"},
  {label: 'Tình trạng', key: "status"},
  {label: 'Nguồn cấp', key: "device"},
]

export const Report = () => {

  const reportsState = useSelector((state: RootState) => state.report.reports);
  const reportsFilterState = useSelector((state: RootState) => state.report.reportsFilter);
  ...
  const pageCount = () => {
    if (reportsFilterState.length === 0) return Math.ceil(reportsState.length / PER_PAGE);
    return Math.ceil(reportsFilterState.length / PER_PAGE);
  }
  ...
  const handleFilterDate = useCallback((value: { dateStart: string, dateEnd: string }) => {
    ...
    const res: report[] = [];
    reportsState.forEach((item: report) => {
      const date = getOnlyDate(item.date, "ahourAndDAte");
    ...
    return dispatch(updateReportsFilter(res));
  }, [reportsState, dispatch,])
      ...
      <div className={styles.body} >
        ...
        <div>
          <table>
            ...
            <tbody>
              {reportsFilterState.length === 0 && reportsState.slice(offset, offset + PER_PAGE).map((item) => (
                <tr key={item.number} >
                  <td>{item.number}</td>
                  <td>{item.name}</td>
                  <td>{item.date}</td>
              ...
              {reportsFilterState.length > 0 && reportsFilterState.slice(offset, offset + PER_PAGE).map((item) => (
                <tr key={item.number} >
                  <td>{item.number}</td>
                  <td>{item.name}</td>
                  <td>{item.date}</td>
        ...
        <CSVLink
          data={reportsFilterState.length === 0 ? reportsState : reportsFilterState}
          headers={headers}
          filename={"report.csv"}
        >
