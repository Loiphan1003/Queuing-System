...
import { updateDataDashboard, updateFilterDashboard } from '../../store/reducers/dashboardSlice';
...
export const Dashboard = () => {
  ...
  const checkExistTest = (list: { label: string, value: number }[], value: string) => {
    let status: boolean = false;
    ...
    if (status === false && new Date(value).getFullYear() === currentDate.getFullYear()) list.push({
      label: value,
      value: 1
    })
    return;
  ...
  const filterDatesByWeekAndMonth = (dates: Date[]): { [month: number]: { [weekNumber: number]: Date[] } } => {
  ...
  const getMonthAndNumberOfWeeks = (filteredDates: { [month: number]: { [weekNumber: number]: Date[] } }) => {
  ...
  const displayWithWatchType = useCallback(() => {
    ...
    const filteredDates: { [month: number]: { [weekNumber: number]: Date[] } } = filterDatesByWeekAndMonth(dates);

    const entries = getMonthAndNumberOfWeeks(filteredDates)
    const getValueAccordingToCurrentMonth = entries.filter((item) => { return item.month === (currentDate.getMonth() + 1) })
