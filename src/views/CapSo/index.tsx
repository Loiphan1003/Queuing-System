...
import { addNumberLevel, getAllNumberLevels, setNumberLevelsFilter, updateIsFilterNumber } from '../../store/reducers/numberLevelSlice';
...
export const CapSo = () => {
  ...
  const isFilterNumberState = useSelector((state: RootState) => state.numberLevel.isFilterNumber);
  const numberLevelsFilterState = useSelector((state: RootState) => state.numberLevel.numberLevelsFilter);
  ...
  const pageCount = () => {
    if (!isFilterNumberState) return Math.ceil(numberLevelsState.length / PER_PAGE);
  ...
  const removeDuplicate = (collection: device[], isDuplicate: IsDuplicate) =>
    collection.filter((item, index, items: device[]) =>
      items.findIndex(secondItem => isDuplicate(item, secondItem)) === index)

  const displayDeviceForSelectBox = () => {
    let devicesName: string[] = ["Tất cả"];
    const removeDup = removeDuplicate(devicesState, (a, b) => a.deviceName === b.deviceName)
    removeDup.forEach((item) => { return devicesName.push(item.deviceName) })
    return devicesName;
  }

  const handleFilter = (value: string, type: string) => {
    if (value === '' || value.match("Tất cả")) return dispatch(updateIsFilterNumber(false));
    let res: NumberLevel[] = [];
    if (type.match('service')) res = numberLevelsState.filter((item) => { return item.service.match(value) })
    if (type.match('status')) res = numberLevelsState.filter((item) => item.status.match(value));
    if (type.match('device')) res = numberLevelsState.filter((item) => item.device.match(value));
  ...
  const handleFilterDate = useCallback((value: { dateStart: string, dateEnd: string }) => {
    ...
    numberLevelsState.forEach((item: NumberLevel) => {
        ...
        if (
            ...
            ||
            (dateEnd.getDate() >= dateExpire.getDate() && dateEnd.getMonth() >= dateExpire.getMonth() && dateEnd.getFullYear() >= dateExpire.getFullYear())
        ) {
  ...
  const handleSearchText = useCallback((text: string) => {
    ...
    const res = numberLevelsState.filter((item) => {
      return item.stt.toString().includes(text) || item.customer.includes(text)
    })
    dispatch(updateIsFilterNumber(true))
    dispatch(setNumberLevelsFilter(res))
          ...
          <div className={styles.body} >
            <div>
              <table>
                ...
                <tbody>
                  {!isFilterNumberState && numberLevelsState.slice(offset, offset + PER_PAGE).map((item) => (
                    <tr key={item.stt} >
                      <td>{item.stt}</td>
                      <td>{item.customer}</td>
                      <td>{item.service}</td>
                  ...
                  {isFilterNumberState && numberLevelsFilterState.slice(offset, offset + PER_PAGE).map((item) => (
                    <tr key={item.stt} >
                      <td>{item.stt}</td>
                      <td>{item.customer}</td>
                      <td>{item.service}</td>
